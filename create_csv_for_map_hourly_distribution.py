...
def daily_distribution_month_over_weekday(month: str, day_of_week: int):
    """
    :param month: format yyyymm
    """
    if int(month) < 201700:
    ...
    if 'endtime' in data:
        ...
        data['end_hour'] = data['ended_at'].dt.hour
    if 'starttime' in data:
        data = data[(data['starttime'].dt.dayofweek == day_of_week) | (data['endtime'].dt.dayofweek == day_of_week)]
    else:
        data = data[(data['started_at'].dt.dayofweek == day_of_week) | (data['ended_at'].dt.dayofweek == day_of_week)]
    start_hourly_counts = data.groupby(['start_hour', 'start_station_name']).size().reset_index(name='count')
    end_hourly_counts = data.groupby(['end_hour', 'end_station_name']).size().reset_index(name='count')
    start_hourly_counts = start_hourly_counts.rename(columns={'start_station_name': 'station_name', 'start_hour': 'hour'})
    end_hourly_counts = end_hourly_counts.rename(columns={'end_station_name': 'station_name', 'end_hour': 'hour'})
    ...
    merged_data = merged_data.rename(columns={'count_x': 'start_count', 'count_y': 'end_count'})
    merged_data['deficit'] = merged_data['start_count'] - merged_data['end_count']
    merged_data['deficit'] = merged_data['deficit'] // 5
    merged_data['popularity'] = merged_data['start_count'] + merged_data['end_count']
    merged_data['popularity'] = merged_data['popularity'] // 5
    merged_data.to_csv('output_files/csv_files/' + month + '_hourly_rent_return_' + str(day_of_week) + '.csv')
    # Clean up - remove the temporary extracted folder
    os.remove(csv_path)
    os.rmdir('temp_extracted_folder')
...
# daily_distribution_month_over_weekday('202207', day_of_week=0)
daily_distribution_month_over_weekday('202201', day_of_week=0)
